...
- **Vanilla-ts runner fix (session 4):** `buildVanillaTsRunner()` transforms test files at runtime: strips type-only imports, converts static `import { x } from './index'` to a dynamic `await import('./index').catch(() => ({}))` inside an async IIFE. Parcel bundles ESM natively — a static named import from a file with no matching export throws a `SyntaxError` at module evaluation time, killing the runner before any `console.log` runs. With dynamic import, missing exports safely become `undefined` and tests run, correctly reporting each failure.
